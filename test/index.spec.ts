...
import { GetBlockchainInfoResponse } from "../pb/bchrpc_pb";
...
describe("grpc-bchrpc-browser", () => {
    ...
    it("getRawTransaction returns a serialized raw tx with matching hash", async () => {
        ...
        assert.equal(hash64, util.u8toBase64(hashTwoU8), "check that raw transaction matches it's hash");
    });
    ...
    it("getMerkleRootFromProof should build merkle tree", async () => {
        ...
        const block15000 = new Map<number, string[]>();
    ...
    it("hashPair returns sha256(sha256(ab)) from hex", async () => {
        ...
        assert.deepEqual(abU8, hashPairResult);
    })
    ...
    it("hashPair returns calculates merkle root", async () => {
        ...
        const abcdefghijResult = await util.hashPair(
            await util.hashPair(
                await util.hashPair(
                    ab,
                    await util.hashPair(
        ...
        assert.deepEqual(abcdefghij, abcdefghijResult);
    })
